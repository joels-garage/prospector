/*
 * Copyright 2008 Joel Truher Author: Joel Truher (joel@truher.org)
 */
package com.joelsgarage.show.action;

import net.sourceforge.stripes.action.UrlBinding;

import com.joelsgarage.model.ClassPreference;
import com.joelsgarage.prospector.client.model.ModelEntity;

@UrlBinding(value = "/class-preference-list")
public class ClassPreferenceListActionBean extends ModelEntityListActionBean {
	@Override
	protected String getJSP() {
		return "/class-preference-list.jsp"; //$NON-NLS-1$
	}

	@Override
	public Class<? extends ModelEntity> getPersistentClass() {
		return ClassPreference.class;
	}
}
